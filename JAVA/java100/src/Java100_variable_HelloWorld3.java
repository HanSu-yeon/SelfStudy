...
/**
 접근제한자 클래스선언 클래스이름 {
    접근제어(한)자 static 반환타입 메인메서드(문자열 배열 변수명) {
        //구현할 코드 작성
    }
 }
 ...
 (1)메인메서드: main()
            ...
            -길게 작성된 소스에서 그 프로그램의 시작이 어딘지 알 수 없으면 안되므로 시작점을 알려주는 용도로도 사용 -->entry point == main()
 (2)파라미터스: String[] args
            -메서드(함수) 호출시 하나 or 둘 이상의 파라미터 값을 넣어서 호출할 수 있음
            -그러한 인수(파라미터)들의 값을 저장할 변수(바구니)들을 명시
            -String은 문자열, []는 배열을 의미. args == argument(인수,독립 변수)
            -args는 하나의 변수명일 뿐이므로 임의의 이름을 지정해도 무방
 (3)반환할타입: return type
            -반환할 값이 있나?없냐? -->없으면 void(빈 공간, 공허함, empty)
            -이 메서드(함수)는 호출하면 결과로써 특별히 반환되는 값 없이 수행되는 메서드
* */