...
public class Java100_ex08 {
    public static void main(String[] args) {
        //1."추상 클래스는" 구체적인 내용이 없기 때문에 "객체를 생성할 수 없다"
//        Animal dog = new Animal(); 에러남
        ...
        //3. Summary
        /*
        - 추상(부모) 클래스는 다른(자식) 클래스들의 공통적인 특징을 변수나 메서드로 정의만 해놓은 것을 말한다 --> 추상 메서드
        - abstract를 앞에 붙이고 클래스 안에 추상 메서드를 포함하고 있다는 것을 제외하면 일반 클래스와 별반 차이가 없다
        - Field, Constructor, Method(추상 메서드 말고 일반 메서드)도 포함할 수 있다
        - 메서드 선언만 있고 구체적인 내용은 없으므로 객체를 생성할 수 없다
        - 따라서, 부모 클래스로서의 역할은 하지만, **구체적인 사용은 상속받은 자식 클래스에서 재정의(오버라이딩)하여 사용**해야 한다 --> 강제성
        ...
        - 위의 cat클래스에서 재정의한 cry()메서드를 지워보자 -->에러나는 이유는 추상 메서드를 재정의(오버라이딩)하지 않아서이다

        꼭 재정의(override)해야만 하는가?
        - 일단 그렇다
        - 자식 클래스에서 부모(추상) 클래스로부터 상속받은 추상 메서드는 일단은 무조건 오버라이딩해서 재정의 한 후 사용해야 한다
        - 추상 메서드를 포함하고 있다면 곧 추상 클래스여야 한다

        만약,재정의 하고 싶지 않다면?
        - 자식 클래스에서 상속받은 추상 메서드를 구현하지 않는다면 자식 클래스에도 abstract를 붙여서 추상으로 만들어준다
